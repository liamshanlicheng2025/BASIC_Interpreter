...
ParsedLine::ParsedLine() {
    line_number_ = -1;
    has_line_number_ = false;
    statement_ = nullptr;
}

ParsedLine::~ParsedLine() { delete statement_; }

void ParsedLine::setLine(int line) {
    line_number_ = line;
    has_line_number_ = true;
}

bool ParsedLine::hasLine() const {
    return has_line_number_;
}
int ParsedLine::getLine() const {
    return has_line_number_ ? line_number_ : -1;
}
...
Statement* Parser::parsePrint(TokenStream& tokens,
                              const std::string& originLine) const {
    auto expr = parseExpression(tokens);
    return new PrintStmt(expr, originLine);
    // TODO: create a corresponding stmt and return it.
}
...
Statement* Parser::parseIf(TokenStream& tokens,
    ...
    int targetLine = parseLiteral(lineToken);
    return new IfStmt(leftExpr, op, rightExpr, targetLine, originLine);
    // TODO: create a corresponding stmt and return it.
}
